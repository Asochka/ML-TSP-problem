function SimulatedAnnealing2()
    ...
    state2Opt = OptimiseRoute_2Opt(cities, initialTemperature, endTemperature); % call our optimization function 2-Opt
    figure
    plot(cities(state2Opt,1),cities(state2Opt,2),"g--o"); % draw final route
    title('2-Opt route')
end 
...
function [state] = OptimiseRoute(cities, initialTemperature, endTemperature)
    ...
    disp('Final route length with old algorithm: ');
    disp(currentEnergy);
end
...
function [ state ] = OptimiseRoute_2Opt(cities, initialTemperature, endTemperature)
    nCities = size(cities,1);
    state = [1:nCities]'; % setup initial cities visit order as numbered column-vector (transposed row)
    currentEnergy = CalculateEnergy2(state, cities); % calculate the energy for the initial condition
    T = initialTemperature;
    
    for k = 1:100000 % main loop
        stateCandidate = GenerateStateCandidate_2Opt(state); % create a new order for visiting cities
        candidateEnergy = CalculateEnergy2(stateCandidate, cities); % calculate its energy
    ...
    disp('Final route length with 2-Opt: ');
    disp(currentEnergy);
end
...
function [ E ] = CalculateEnergy2(sequence, cities) % calculate route length
   n = size(sequence,1);
   E = 0;
   for i = 1:n-1
